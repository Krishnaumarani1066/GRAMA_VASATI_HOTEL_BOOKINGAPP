...
fun AuthenticationScreen(
    ...
    onNavigateToHome: () -> Unit,      // ✅ ADD THIS to your nav graph call
    ...
    // ✅ FIX 1: Use LaunchedEffect with state key, not wrapped in if()
    ...
    // ✅ FIX 2: Navigate new user to fill profile
    LaunchedEffect(state.signedUpNewUser) {
        if (state.signedUpNewUser) {
            onNavigateToFillProfile()
        }
    }

    // ✅ FIX 3: Navigate existing user to home
    ...
    // ✅ FIX 4: Simple error toast
    LaunchedEffect(state.signInWithGoogleError) {
        if (state.signInWithGoogleError) {
            Toast.makeText(
                context,
